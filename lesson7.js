...
var block_timer; // Таймер для блока
...
function move() {
    ...
    if (!isSnakeUnit(new_unit)) {
	   ...
	   if (!haveFood(new_unit) && !haveBlock(new_unit)) {
            removedTail();
        }
    }
    }
...
//     while (!blockCreated) { //пока блок не создали
//         // рандом
